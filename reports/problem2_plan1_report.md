...
- Analysis unit: 孕妇
- Verification status: VERIFIED（固定种子三次完整运行；四份核心CSV逐字节一致；结果契约和图形检查通过）
- Runtime: 67.65--73.15秒（三次分别为73.15、67.65、69.79秒）
...
`q2.csv`保留既有12列文件结构。为兼容历史契约，字段`t_p0.80_median`现在存放联合优化的组时点，即组内个体t0.80的80%分位数，而非中位数。
...
2. 推荐时点随BMI单调推迟，形成10.0、12.4、16.1、19.3和25.0周的可执行阶梯。
3. G5的25周是删失上界，不能表述为该组已有80%覆盖保证；模型实际只能确认57.5%。
