...
## 11. 决策层修订 R2（人类终审，覆盖第 3/5/6/7 节的相应内容）

> 重要：以下修订优先于第 3/5/6/7 节中的冲突内容。概率模型、MC 抽样、bootstrap、
> 内部校准、s6/s7 敏感性全部原样保留；本修订只改决策层定义与主交付。

### 11.1 诊断依据（须写入报告）

当前旧版主结果（G1 12.024 周 / G2 12.034 周，差 0.011 周，distinct_required=False）
不构成"数据支持统一约 12 周检测"的证据。压平由决策损失造成，而非数据或概率模型：

- 旧损失函数 r_delay(t)=0（t≤12）形成零风险平台；模型达标概率斜率最大约 0.062/周，
  小于斜坡斜率 1/15，ρ=1 时损失最优点对所有 BMI 都钉在 12 周拐角。
- 四种概率源（简约分段 Beta、Q1 无交互样条、分位数反演、含交互样条）的最优点全部
  落在 12.0–12.5 周，证实压平与模型族无关。
- 同一模型内，达标时间反演 t_p0.80 随 BMI 单调分层：组中位曲线 G1≈11.6 周、
  G2≈12.3 周；个体层面 t_p80 与 BMI 相关约 0.82（BMI 20.7→10.0 周，BMI 28.4→12.2 周）。

### 11.2 主交付改为达标时间反演 t_p

- 定义：t_p(b) = inf{t ∈ {10.0, 10.1, …, 25.0} : p_marg(t,b) ≥ p}，取首次穿越，
  不做 PAVA/isotonic 后处理。
- 主阈值 p=0.80。组推荐时点 = 组内孕妇 t_p0.80 的中位数，附孕妇层 cluster bootstrap
  95% 区间（B=100，沿用现有流程）。
- 稳健性说明（报告内一句话）：p=0.85 时两组均后移且 G1<G2 方向不变，但具体周数受
  12.5–20 周平台段影响（G2 可达 23.5 周），故主结论只宣称"高 BMI 组达标时间更晚"的
  分层方向，周数以 0.80 为准；p=0.90 对高 BMI 组中位曲线在窗口内不可行，报告为
  "≥25 周/无解"。

### 11.3 风险函数去零平台，t* 降为二级视图

- r_delay(t) = (t−10)/(27−10)，t∈[10,27]；28 周档仍为域外解释，不参与优化。
- t* 只保留一版：ρ=1、t0=10、0.1 周网格 + 三点抛物线细分，作为"风险平衡时点"
  参考列；不做 ρ、t0 扫描。
- 当前组中位曲线下 t*≈10.8（G1）/11.7（G2）周，以程序重算为准。

### 11.4 判定与分组

- distinct_required：组间 t_p0.80 中位数差 ≥ 0.5 周记 True（≈3.5 天，低于此值属排程
  噪声；当前约 0.7 周），同时输出两组 bootstrap 区间；若区间重叠，在报告中如实说明
  不确定性，不改变主结论方向。
- 分组沿用现有 BMI 监督分箱（n_min=20，K=2 默认），边界 bootstrap 重现频率照旧报告，
  边界不作为临床截点。
- 双通道误差：通道 A 改为报告 σ=σ_tech 下 t_p 与 t* 的偏移及区间宽度；通道 B 给出
  t_p 的 bootstrap 区间。内部校准与数据质量/GC 敏感性（s6、s7）保留。

### 11.5 输出变更

- 主结果表列：group, bmi_low, bmi_high, n, median_bmi, t_p0.80_median, ci_low,
  ci_high, t_star, distinct_required, delta_t_sigma_tech。
- 图：个体 t_p0.80 vs BMI 散点 + 分组中位数线；两组 t_p0.80 与 95% CI 对比；新损失
  曲线（斜坡 t0=10）与 t*。
- 报告结论改写：删除"数据支持统一约 12 周检测"，改为"低 BMI 组建议约 11.5–12 周、
  高 BMI 组约 12.3 周（80% 达标保证）；旧'统一 12 周'结论系损失函数零风险平台所致，
  非数据结论"。
